...
//     const cartTotalElement = document.querySelector(".totalprice.price strong"); // Selector elemen
//     const formattedTotal = cartTotalElement.innerText || cartTotalElement.textContent;

//     console.log(formattedTotal);
    
//     // Bersihkan format dan ubah ke angka
//     const cartTotalNumber = parseFloat(formattedTotal.replace("Rp", "").replace(",", "."));

//     // Debugging: Tampilkan di console
//     console.log(cartTotalNumber); // Output: 33.00
    
// });

//------------------------- SUKSES TARIK DATA DAN TAMPILKAN KE SNAP
// document.addEventListener('DOMContentLoaded', function () {
//     // Ambil elemen total harga dan tombol pembayaran
//     const cartTotalElement = document.querySelector(".totalprice.price strong"); // Selector elemen total
//     const payButton = document.getElementById('paybuttonmidtrans'); // Selector tombol paybuttonmidtrans
...
//         console.log("Total Amount:", cartTotalNumber); // Debugging untuk memastikan nilai benar
        
//         // Tambahkan event listener pada tombol
//         payButton.addEventListener('click', function (event) {
//             event.preventDefault(); // Cegah submit default form (jika tombol dalam form)
...
//                 },
//                 body: JSON.stringify({ totalAmount: cartTotalNumber }), // Kirim data totalAmount
//             })
//                 .then(response => response.json())
//                 .then(data => {
//                     if (data.status === 'success') {
//                         console.log("Transaction successful, token:", data.transaction_token);
//                         window.snap.pay(data.transaction_token); // Memanggil Midtrans Snap API
//                     } else {
...
//         });
//     } else {
//         console.log("Element 'cartTotalElement' atau 'payButton' tidak ditemukan.");
//     }
// });
    
//------------------------- OUTPUT DALAM BENTUK RIBUAN 
document.addEventListener('DOMContentLoaded', function () {
    // Ambil elemen total harga dan tombol pembayaran
    const cartTotalElement = document.querySelector(".totalprice.price strong"); // Selector elemen total
    const payButton = document.getElementById('paybuttonmidtrans'); // Selector tombol paybuttonmidtrans
    ...
    if (cartTotalElement && payButton) {
        ...
        console.log("Total Amount:", cartTotalNumber); // Debugging untuk memastikan nilai benar
        console.log("Total Amount In Thousands:", cartTotalInThousands); // Debugging untuk memastikan nilai benar
        ...
        payButton.addEventListener('click', function (event) {
            ...
            fetch('/control/createpaymentmidtrans', {
                ...
                body: JSON.stringify({ totalAmount: cartTotalInThousands }), // Kirim data totalAmount
            })
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'success') {
