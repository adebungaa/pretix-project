...
db_backend = config.get('database', 'backend', fallback='postgresql')
...
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.' + db_backend,
        'NAME': config.get('database', 'name', fallback='dbpretix'),  # Nama database PostgreSQL
        'USER': config.get('database', 'user', fallback='pretix'),  # Username untuk PostgreSQL
        'PASSWORD': config.get('database', 'password', fallback='pretix'),  # Password PostgreSQL
        'HOST': config.get('database', 'host', fallback='localhost'),  # Host, biasanya localhost
        'PORT': config.get('database', 'port', fallback='5432'),  # Port PostgreSQL, default 5432
        'CONN_MAX_AGE': 120,  # Optimal connection reuse for PostgreSQL
        'CONN_HEALTH_CHECKS': True,  # Kesehatan koneksi akan dicek (Django 4.1+)
        'OPTIONS': db_options,
        'TEST': {} 
    }
}
